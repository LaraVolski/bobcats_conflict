...
ggplot(subset.rai, aes(x = Sheep, y = Bobcat_RAI, fill = Sheep)) +
  ...
  scale_fill_manual(values=c("cadetblue4", "orange1")) +
  ggsave("figures/bobcat_sheep_fire.png", width = 4, height = 4)
...
ggplot(rai_bobcat_lv, aes(x = Rugged.121, y = RAI)) +
  geom_smooth(method = lm) +
  ggsave("figures/rugged121_foxlightsdata.png", width = 4, height = 4)

ggplot(rai_bobcat_lv, aes(x = Elevation, y = RAI)) +
  geom_smooth(method = lm) +
  ggsave("figures/elevation_foxlightsdata.png", width = 4, height = 4)

ggplot(rai_bobcat_lv, aes(x = road.dist.clean, y = RAI)) +
  geom_smooth(method = lm) +
  ggsave("figures/road.dist.clean_foxlightsdata.png", width = 4, height = 4)

ggplot(rai_bobcat_lv, aes(x = Slope, y = RAI)) +
  geom_smooth(method = lm) +
  ggsave("figures/slope_foxlightsdata.png", width = 4, height = 4)
...
ggplot(subset.rai, aes(x = Individual_Dog, y = Count)) +
...
ggplot(rai_bobcat_lv, aes(x = Ind_Dogs, y = Detections)) +
  geom_col(fill = "cadetblue4") +
  labs(x = "Dogs", y = "Bobcat Detections")

ggplot(data = rai_bobcat_lv, aes(x = Ind_Dogs, y = RAI * 35, group = Ind_Dogs)) +
  geom_boxplot(alpha=2, fill = "cadetblue4") +
  labs(x ="Dogs", y = "Bobcat RAI") +
  theme(legend.position = "none") +
...
## Question 2 - Diel Graph for LGD
```{r}

write.csv(record.table.all,"record.table.all.csv", row.names = TRUE)




## taken from foxlights code
# Bobcat Diel Patterns w/ and w/o LGD
...
```{r}
x <- subset(all.rai, Class == "Bobcat" & Camera == "G11")$RAI
y <- mean(x, na.rm = TRUE)
print(y)


x <- subset(subset.rai, Season == "Winter" & Camera == "G11")$Bobcat_RAI
y <- mean(x, na.rm = TRUE)
print(y)
