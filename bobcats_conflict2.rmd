...
ggplot(subset.rai, aes(x = Season, y = Bobcat_RAI, fill = Season)) +
  ...
  labs(x ="Season", y = "Bobcat Detection Rate") +
  theme_bw()
  ggsave(file = "bobcat.seasonality.png", width = 4, height = 4)
...
ggplot(subset.rai, aes(x = Season, y = Bobcat_RAI, fill = Season)) +
  ...
  #scale_y_log10() +
  theme_bw()
  ggsave(file = "bobcat.seasonality.boxplot.png", width = 4, height = 4)
...
mean(subset(subset.rai, Sheep == "N")$Bobcat_RAI)

sd(subset(subset.rai, Sheep == "Y")$Bobcat_RAI)

## Graph Bobcat RAI by Sheep Presence and Fire

## to make graph w sheep as opposed to grazing, use 'subset.rai.fire.year.csv' and 'Sheep' instead of 'Grazed'

subset.fire.rai <- read.csv("subset.rai.fire.year.csv")

  ggplot(subset.fire.rai, aes(x = Sheep, y = Bobcat_RAI, fill = Sheep)) +
  geom_boxplot() +
  theme_bw() +
  theme(legend.position = "none") +
  ...
  scale_y_log10() +
  facet_wrap(~ Fire) +
  #labs(x ="Burned and Fire", y = "Relative Bobcat Activity (detections/month)") +
  scale_fill_manual(values=c("cadetblue4", "orange1")) 
  ggsave(file = "bobcat.rai.by.fire.and.grazing.png")

# Chi Square Test of Bobcats/Grazing/Fire
  mean(subset(x = subset.fire.rai, Sheep == "N" & Fire == "N")$Bobcat_RAI) 
  mean(subset(x = subset.fire.rai, Sheep == "Y" & Fire == "N")$Bobcat_RAI)
  mean(subset(x = subset.fire.rai, Sheep == "Y" & Fire == "Y")$Bobcat_RAI)
  mean(subset(x = subset.fire.rai, Sheep == "N" & Fire == "Y")$Bobcat_RAI)
  
  yay <- c(0.01640267, 0.0056773355, 0.1771093406, 0.0000008916519309)
  yay <- matrix(data = yay, nrow = 2, ncol = 2)
  chisq.test(yay)
  

bob.by.burn.and.grazing <- read.csv("bob.by.burn.and.grazing.csv")

  ggplot(bob.by.burn.and.grazing, aes(x = Grazed, y = Bobcat_RAI, fill = Grazed)) +
  geom_boxplot() +
  theme_bw() +
  theme(legend.position = "none") +
  scale_x_discrete(labels = c("Ungrazed", "Grazed")) +
  scale_y_log10() +
  facet_wrap(~ Fire) +
  labs(x ="", y = "Bobcat Detection Rate") +
  scale_fill_manual(values=c("cadetblue4", "orange1")) 
  ggsave(file = "bobcat.rai.by.fire.and.grazing.png")
  
  
#average detections by camera

sum(subset(subset.rai, Camera == "B04")$Count)

mean(subset(subset.rai, Camera == "B04")$Bobcat_RAI)
...
geom_jitter() +
labs(title = "Bobcat Detection Rate by Active Foxlight Period",
       x = "First, Second, Third, Fourth, or Fifth Time a Foxlight was Active at a Site", y = "Bobcat Detection Rate") +
  theme_bw() 

ggsave("bobcat.activity.by.active.foxlight.phase.png")
...
labs(x ="Week", y = "Median Bobcat Detection Rate") +
  scale_fill_manual(values=c("cadetblue4", "orange1")) + 
  theme_bw()
...
ggplot(subset.rai, aes(x = Sheep, y = Bobcat_RAI, fill = Sheep)) +
  ...
  scale_fill_manual(values=c("cadetblue4", "orange1")) 
ggsave("sheep.and.bobcat.rai.boxplot.lgd.png")

# Individual Dog Count using Grid Camera Survey data
ggplot(data = subset.rai, aes(x = Individual_Dog, y = Bobcat_RAI, group = Individual_Dog)) +
...
# plot for bobcats
multilines2(records.bobcat.sheepyes$Time.Sun, records.bobcat.sheepno$Time.Sun, main = "Effect of Sheep and LGD presence on bobcat diel activity patterns")
legend('top', c("Present", "Absent"), col = c("orange1", "cadetblue4"), lty=c(1,1), bty='n')
...
(Dhats <- overlapEst(records.bobcat.sheepyes$Time.Sun, records.bobcat.sheepno$Time.Sun))
...
bs_time.sheepyes <- resample(records.bobcat.sheepyes$Time.Sun, 10000)
bs_time.sheepno <- resample(records.bobcat.sheepno$Time.Sun, 10000)
...
bsOut <- bootEst(bs_time.sheepyes, bs_time.sheepno)
colMeans(bsOut) 
...
#    Dhat1     Dhat4     Dhat5 
#0.7919166 0.7910264 0.7571053 
#    Dhat1     Dhat4     Dhat5 
#0.7924268 0.7903629 0.7976098 
#           lower     upper
#norm   0.6859764 0.8974034
#norm0  0.6853129 0.8967399
#basic  0.6895368 0.9013974
#basic0 0.6813189 0.8931796
#perc   0.6806554 0.8925160
```

```{r}
record.table.lgd2 <- read.csv("lgd.diel.graph.record.table.with.dogs.csv")

record.table.lgd2$Date <- format(as.Date(as.character(record.table.lgd2$Date), "%m/%d/%y"))

# Setting up
# specify date format
record.table.lgd2$Date <- as.POSIXct(record.table.lgd2$Date)
...
Coords <- sp::SpatialPoints(coords,
                            proj4string=sp::CRS("+proj=longlat +datum=WGS84"))
# store time as hms
record.table.lgd2$Time.Corrected <- hms(record.table.lgd2$Time)

# convert time from HH:MM:SS to decimal hours (HH.HHHH or whatever)
record.table.lgd2$Time.Decimal <- record.table.lgd2$Time.Corrected$hour + record.table.lgd2$Time.Corrected$minute/60 + record.table.lgd2$Time.Corrected$second/3600

# scale time so that it is between 0 and 1 by dividing by 24
record.table.lgd2$Time.Scaled <- record.table.lgd2$Time.Decimal / 24

# convert scaled time (0 to 1) to radians (0 to 2pi)
record.table.lgd2$Time.Radians <- record.table.lgd2$Time.Scaled * 2 * pi

# calculate suntime using function from overlap package, and coordinates and dates as formatted above
record.table.lgd2$Time.Sun <- sunTime(record.table.lgd2$Time.Radians, record.table.lgd2$Date, Coords)

# plot coyote activity with and without foxlights
records.bobcat.bobcat <- subset(record.table.lgd2, Species == "Bobcat")
records.bobcat.dog <- subset(record.table.lgd2, Species == "Dog")

# define overlap function
multilines2 <-function (A, B, xscale = 24, linetype = c(1, 2), linecol = c("orange1", "white"), n.grid = 128, kmax = 3, adjust = 1,
                        ...) 
{
  bwA <- getBandWidth(A, kmax = kmax)/adjust
  bwB <- getBandWidth(B, kmax = kmax)/adjust
...
watson.two.test(records.bobcat.bobcat$Time.Sun, records.bobcat.dog$Time.Sun)
...
# plot for bobcats
multilines2(records.bobcat.bobcat$Time.Sun, records.bobcat.dog$Time.Sun, main = "Bobcat Activity vs LGD Activity")
legend('bottom', c("Bobcat Diel Pattern", "LGD Diel Pattern"), col = c("orange1", "cadetblue4"), lty=c(1,1), bty='n')

#test significance
# this will return the degree of overlap - area under the shared curve (where 0 is completely different and 1 is completely the same)
(Dhats <- overlapEst(records.bobcat.bobcat$Time.Sun, records.bobcat.dog$Time.Sun))
# use Dhat4 (they are different formulas - this one is good for sample sizes >50)
# then resample each one 10,000 times (bootstrapping)
bs_time.bobcat <- resample(records.bobcat.bobcat$Time.Sun, 10000)
bs_time.dog <- resample(records.bobcat.dog$Time.Sun, 10000)
# now calculate the resulting bootstrapped overlap value
# it should be pretty similar to the "true" value (I think it makes more sense to report the true value, with the bootstrapped confidence intervals)
bsOut <- bootEst(bs_time.bobcat, bs_time.dog)
colMeans(bsOut) 
# now calculate the 95% confidence interval - this takes a little while to run
bs <- as.vector(bsOut[,2])
(bsCI_inout <- bootCI(Dhats[2], bs))
...
```{r}
sum(subset(all.rai, Class == "Bear")$Count, na.rm = TRUE)
```

# A t-test for seasonality
```{r}

# Summer T-Test with Count Data
set.seed(1997)
bobcat.summer <- (subset(subset.rai, Season == "Summer"))
sum(bobcat.summer$Count) # 79
mean(bobcat.summer$Count) # 0.15
sd(bobcat.summer$Count) # 0.5820631

mean(subset.rai$Count) # for the mu!

bobcatSummer <- c(rnorm(80, mean = 0.15, sd = 0.5820631))
t.test(bobcatSummer, mu = 0.3317215) # p-value 2.027e-06

# Summer T-Test with RAI Data
set.seed(1997)
bobcat.summer <- (subset(subset.rai, Season == "Summer"))
mean(bobcat.summer$Bobcat_RAI) # 0.005254837
sd(bobcat.summer$Bobcat_RAI) # 0.01990193

mean(subset.rai$Bobcat_RAI) # for the mu!

bobcatSummer <- c(rnorm(80, mean = 0.005254837, sd = 0.01990193))
t.test(bobcatSummer, mu = 0.01145354) # p-value 2.081e-06

# Winter T-Test with Count Data
set.seed(1997)
bobcat.winter <- (subset(subset.rai, Season == "Winter"))
sum(bobcat.winter$Count) # 252
mean(bobcat.winter$Count) # 0.4736842
sd(bobcat.winter$Count) # 1.22945

bobcatWinter <- c(rnorm(261, mean = 0.4736842, sd = 1.22945))
t.test(bobcatWinter, mu = 0.3317215) # p-value 0.8538

# Winter T-Test with RAI Data
set.seed(1997)
bobcat.winter <- (subset(subset.rai, Season == "Winter"))
sum(bobcat.winter$Bobcat_RAI) # 252
mean(bobcat.winter$Bobcat_RAI) # 0.01676567
sd(bobcat.winter$Bobcat_RAI) # 0.04432347

bobcatWinter <- c(rnorm(261, mean = 0.01676567, sd = 0.04432347))
t.test(bobcatWinter, mu = 0.01145354) # p-value 0.8006
```
