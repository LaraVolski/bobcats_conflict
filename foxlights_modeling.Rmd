...
fit10.1 <- glmer.nb(Detections ~ rugged49.clean + offset(log(Operation)) + (1|Camera), data = subset(rai, Species == "LYRU"), control=glmerControl(optimizer="bobyqa"))
summary(fit10)
tab_model(fit10)
```
...
AIC = 328.1
```{r}
fit16 <- glmer.nb(Detections ~ Dogs_Scaled + offset(log(Operation)) + (1|Camera), data = rai_bobcat_lv)
summary(fit16)
tab_model(fit16)
```
...
AIC = 333.6
```{r}
fit18 <- glmer.nb(Detections ~ Dog_RAI *  Foxlight + offset(log(Operation)) + (1|Camera), data = rai_bobcat_lv, control=glmerControl(optimizer="bobyqa"))
summary(fit18)
vif(fit18)
tab_model(fit18)
```
