...
full.model.lgd <- lmer(Count ~ Sheep_RAI + Sheep + Dog_RAI + Dogs_Scaled + Fire + Season + rugged9.clean + rugged25.clean + rugged49.clean + rugged81.clean + rugged121.clean + road.dist.clean + elevation.clean + slope.clean + vegetation.coarser.clean + offset(log(Operation)) + (1|Camera), data = subset.rai, na.action = "na.fail")
...
reduced.model.lgd <- glmer(Count ~ Sheep + Dog_RAI + Fire + Season + rugged9.clean
                + rugged121.clean + road.dist.clean + elevation.clean
                + slope.clean + vegetation.coarser.clean + offset(log(Operation))
                + (1 | Camera), data = subset.rai, family = "poisson", na.action = "na.fail")

AIC(reduced.model.lgd) ## AIC is 2771.6
vif(reduced.model.lgd) # All is under 2
