...
```{r}
fit.2 <- glmer.nb(Count ~ Season + rugged121.clean + Dog_RAI + offset(log(Operation)) + (1|Camera), data = subset.rai)
vif(fit.2)
summary(fit.2)
tab_model(fit.2)
...
```{r}
fit.3 <- glmer.nb(Count ~ Season * Dog_RAI + offset(log(Operation)) + (1|Camera), data = subset.rai)
vif(fit.3) # Over 2?
summary(fit.3)
tab_model(fit.3)
```
